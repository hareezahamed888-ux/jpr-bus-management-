  ...
  return (
    <div style={{ maxWidth: 420, margin: "0 auto" }}>
      ...
      <form onSubmit={handleSubmit} style={{ background: "#fff", border: "1px solid #E1E4E1", borderRadius: 4, padding: 24 }}>
        <label style={{ display: "block", fontFamily: "'IBM Plex Sans', sans-serif", fontSize: 12, color: "#6B7078", marginBottom: 6 }}>
          {fieldLabels.id}
        </label>
        <input
          value={idValue}
          ...
          style={{ width: "100%", boxSizing: "border-box", padding: "10px 12px", borderRadius: 4, border: "1px solid #D8DCD9", fontSize: 14, fontFamily: "'IBM Plex Sans', sans-serif", marginBottom: 16 }}
        />
        ...
        <input
          ...
          style={{ width: "100%", boxSizing: "border-box", padding: "10px 12px", borderRadius: 4, border: "1px solid #D8DCD9", fontSize: 14, fontFamily: "'IBM Plex Sans', sans-serif", marginBottom: 8 }}
        />

        <div style={{ textAlign: "right", marginBottom: 16 }}>
          <span style={{ fontFamily: "'IBM Plex Sans', sans-serif", fontSize: 12, color: "#8B8F94" }} title="Contact the transport office to reset this">
            Forgot {role === "parent" ? "access code" : "password"}?
          </span>
        </div>

        {error && (
          <div style={{ background: "#F6E4E2", color: "#B3392F", padding: "10px 12px", borderRadius: 4, fontSize: 13, fontFamily: "'IBM Plex Sans', sans-serif", marginBottom: 16 }}>
            {error}
          </div>
        )}
        ...
        <button
          type="submit"
          style={{
            width: "100%", padding: "11px 0", borderRadius: 4, border: "none", cursor: "pointer",
            background: "#1F7A6C", color: "#fff", fontFamily: "'IBM Plex Sans', sans-serif", fontWeight: 600, fontSize: 14,
          }}
        >
          Log in
        </button>
      ...
      <details style={{ marginTop: 14, fontFamily: "'IBM Plex Sans', sans-serif", fontSize: 12, color: "#8B8F94" }}>
        <summary style={{ cursor: "pointer" }}>Demo credentials</summary>
        <div style={{ marginTop: 8, lineHeight: 1.7 }}>
          Student — 24JECS212 / 24JPR@212 (active), 25JEME164 / 25JPR@164 (pending)<br />
          Parent — 24JECS212 / 4041<br />
...
export default function JPRBusApp() {
  ...
  const roleTitle = useMemo(() => {
    if (!session) return "";
    return { student: "Student Portal", parent: "Parent Portal", alumni: "Alumni Portal" }[session.role];
  }, [session]);

  return (
    <div style={{ minHeight: "100%", background: "#F2F4F3", padding: "28px 20px", fontFamily: "'IBM Plex Sans', sans-serif" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@500;700&family=IBM+Plex+Sans:wght@400;500;600&display=swap');
        .pass-card-enter { animation: passIn 220ms ease; }
        @keyframes passIn { from { opacity: 0; transform: translateY(4px); } to { opacity: 1; transform: translateY(0); } }
        input:focus { outline: 2px solid #1F7A6C; outline-offset: 1px; }
        button:focus-visible { outline: 2px solid #1F7A6C; outline-offset: 2px; }
      `}</style>

      <div style={{ maxWidth: 640, margin: "0 auto" }}>
        <div style={{ marginBottom: 22, textAlign: session ? "left" : "center" }}>
          <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 13, color: "#1F7A6C", fontWeight: 700, letterSpacing: "0.03em" }}>
            JPR College Transport Office
          </div>
          <h1 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 26, fontWeight: 700, color: "#14213D", margin: "4px 0 0" }}>
            {session ? roleTitle : "Bus pass portal"}
          </h1>
          {!session && (
            <p style={{ color: "#6B7078", fontSize: 14, margin: "6px 0 0" }}>
              Log in to check your pass status, assigned bus, and route.
            </p>
          )}
        </div>
